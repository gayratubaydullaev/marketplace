...
func (r *OrderRepository) List(tenantID, userID string, customerOnly bool) ([]model.Order, error) {
	...
	err := commondb.WithTenant(r.DB, tenantID, func(tx *sqlx.Tx) error {
		q := `SELECT ` + orderSelect + ` FROM orders WHERE tenant_id=$1`
		args := []any{tenantID}
		if customerOnly {
			q += ` AND user_id=$2`
		...
		return tx.Select(&orders, q, args...)
	})
	return orders, err
}
...
func (r *OrderRepository) GetByNumber(orderNumber, tenantID string) (model.Order, []model.OrderItem, error) {
	...
	err := commondb.WithTenant(r.DB, tenantID, func(tx *sqlx.Tx) error {
		if err := tx.Get(&order, `
			SELECT `+orderSelect+`
			FROM orders WHERE order_number=$1 AND tenant_id=$2`, orderNumber, tenantID); err != nil {
			return err
		}
		return tx.Select(&items, `SELECT `+itemSelect+` FROM order_items WHERE order_id=$1`, order.ID)
	})
...
func (r *OrderRepository) Get(id, tenantID string) (model.Order, []model.OrderItem, error) {
	...
	var items []model.OrderItem
	err := commondb.WithTenant(r.DB, tenantID, func(tx *sqlx.Tx) error {
		if err := tx.Get(&order, `SELECT `+orderSelect+` FROM orders WHERE id=$1 AND tenant_id=$2`, id, tenantID); err != nil {
			return err
		}
		return tx.Select(&items, `SELECT `+itemSelect+` FROM order_items WHERE order_id=$1`, order.ID)
	})
