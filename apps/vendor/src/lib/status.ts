...
export function statusBadgeTone(status?: string): string {
  ...
  switch (s) {
    ...
    default:
      if (s.includes("cancel") || s.includes("reject") || s.includes("block") || s.includes("fail") || s.includes("archiv")) {
        return "bg-rose-50 text-rose-800";
      }
      if (s.includes("pending") || s.includes("unpaid") || s.includes("draft")) return "bg-amber-50 text-amber-800";
